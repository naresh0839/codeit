...
ALGORITHM:: :: :: :: ::
    ...
    by list order(to be exact in reverse order, i.e.in decreasing order of exit times)
        .Every set of vertices,
    reached after the next search, will be the next strongly connected component.

                                   Algorithm asymptotic is O(n + m),
    because it is just two depth(breadth) first searches.
