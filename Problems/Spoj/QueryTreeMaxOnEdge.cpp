// Problem URL :: https://www.spoj.com/problems/QTREE/
...
struct HLD {
    ...
    void solve(string ty) {
        if (ty == "CHANGE") {
            int i, ti;
            cin >> i >> ti;
            int uu = e[i].u, vv = e[i].v;
            int fix = (depth[uu] < depth[vv] ? vv : uu);
            seg.update(1, 0, N - 1, pos[fix], ti);
        } else {
            int l, r;
            cin >> l >> r;
            int baap = lca(l, r), ans;
            if (l == baap or r == baap) {
                int node = (l == baap ? r : l);
                int _node = pth_ancestor(node, depth[node] - depth[baap] - 1);
                ans = query(_node, node);
            } else {
                int node = l;
                int _node = pth_ancestor(node, depth[node] - depth[baap] - 1);
                ans = query(_node, node);
                node = r;
                _node = pth_ancestor(node, depth[node] - depth[baap] - 1);
                ans = max(ans, query(_node, node));
            }
            cout << ans << endl;
        }
    }
